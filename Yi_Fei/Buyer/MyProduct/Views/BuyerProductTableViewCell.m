...
@implementation BuyerProductTableViewCell
...
-(void)buttonClick:(UIButton *)sender{
    ...
    if ([self.delegate respondsToSelector:@selector(clickView:num:)]) {
        [self.delegate clickView:editVC num:self];
    }
        }
            break;
        case 10001:
        {
    BuyerDetailViewController *detailVC = [[BuyerDetailViewController alloc] init];
    if ([self.delegate respondsToSelector:@selector(clickView:row:)]) {
        [self.delegate clickView:detailVC row:self];
    }
  }
            break;
        default:
