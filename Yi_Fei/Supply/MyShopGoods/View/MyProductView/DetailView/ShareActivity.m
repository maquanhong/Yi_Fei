...
-(void)configShareActivity
{
    ...
    NSArray *imagearr = [NSArray arrayWithObjects:@"fasong",@"QQ",@"weixin",@"weibo",@"friend_icon",nil];
    NSArray *titlearray = [NSArray arrayWithObjects:@"发送",@"QQ",@"微信",@"微博",@"朋友圈",nil];
...
{
    if ([self.delegate  respondsToSelector:@selector(clickButtonView:button:)]) {
        [self.delegate clickButtonView:sender.tag button:sender];
    }
    [self tappedCancel];
}
