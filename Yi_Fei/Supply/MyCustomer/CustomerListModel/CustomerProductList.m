...
@implementation CustomerProductList

static CustomerProductList * manager=nil;
+(CustomerProductList *)defaultManager
{
    ...
    dispatch_once(&onceToken,^{
        if(manager==nil){
            manager=[[CustomerProductList alloc]init];
        }
    });
    return manager;
}
...
{
    if(self=[super init]){
        NSString * path=[NSHomeDirectory() stringByAppendingPathComponent:@"Documents/CustomerAskPrice.db"];
        //很据路径创建表
        _dataBase = [[FMDatabase alloc]initWithPath:path];
        //如果创建成功 打开
    if ([_dataBase open]) {
     NSString *createSql = @"create table if not exists CustomerAskPrice(ind integer PRIMARY KEY AUTOINCREMENT,companyID varchar(1024),shopName varchar(1024) ,shopSpecific varchar(1024),shopMed varchar(1024) ,shopSize varchar(1024),shopColor varchar(1024) ,count varchar(1024),flag varchar(1024) ,time  varchar(1024),askTime varchar(1024),shopDescribe varchar(1024),imageOne glob(6000),imageTwo glob(6000),imageThree glob(6000),imageFour glob(6000),customerName varchar(1024),companyName varchar(1024),shopHuoBi varchar(1024),shopTiaoK varchar(1024),shopCustom varchar(1024),shopContent varchar(1024),shopInfo varchar(1024) ,shopPrice varchar(1024),shopAdderss varchar(1024) ,identify varchar(1024))";
...
- (void)insertDataModel:(CustomerProductModel *)model{
    ...
    NSString * insertSql = @"insert into CustomerAskPrice(companyID,shopName,shopSpecific,shopMed,shopSize,shopColor,count,flag,time,askTime ,shopDescribe,imageOne,imageTwo,imageThree,imageFour,customerName,companyName,shopHuoBi,shopTiaoK,shopCustom ,shopContent,shopInfo,shopPrice,shopAdderss,identify) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
    BOOL success=[_dataBase executeUpdate:insertSql,model.companyID,model.shopName,model.shopSpecific,model.shopMed,model.shopSize,model.shopColor,model.count,model.flag,model.time,model.askTime,model.shopDescribe,model.imageOne,model.imageTwo,model.imageThree,model.imageFour,model.customerName,model.companyName,model.shopHuoBi,model.shopTiaoK,model.shopCustom,model.shopContent,model.shopInfo,model.shopPrice,model.shopAdderss,model.identify];
    if (!success) {
        NSLog(@"%@",[_dataBase lastErrorMessage]);
    }else{
        NSLog(@"插入成功");
...
- (NSArray *)getDataName:(NSString*)name and:(NSString*)companyName identify:(NSString*)identify{
    ...
    CustomerProductModel* model = [[CustomerProductModel alloc]init];
        ...
        model.askTime = [set stringForColumn:@"askTime"];
...
-(void)updateMolde:(CustomerProductModel *)model Element:(NSString*)name and:(NSString*)companyName  num:(NSString*)num index:(NSString*)index{
    ...
    NSString *sql = [NSString stringWithFormat:@"update CustomerAskPrice set companyID = ?, shopName = ?,shopSpecific = ?, shopMed = ?, shopSize = ? ,shopColor = ? ,count = ?, flag = ?,time= ?, askTime = ? , shopDescribe = ? ,imageOne = ?,imageTwo = ?,imageThree = ?,imageFour = ?,shopHuoBi = ? ,shopTiaoK = ? ,shopCustom = ? ,shopContent = ?,shopInfo = ?,shopPrice = ? ,shopAdderss = ?   ,identify = ?  where  customerName = ? and  companyName = ? and flag = ? and ind = ?"];
    BOOL success = [_dataBase executeUpdate:sql,model.companyID,model.shopName,model.shopSpecific,model.shopMed,model.shopSize,model.shopColor,model.count,model.flag,model.time,model.askTime,model.shopDescribe,model.imageOne,model.imageTwo,model.imageThree,model.imageFour,model.shopHuoBi,model.shopTiaoK,model.shopCustom,model.shopContent,model.shopInfo,model.shopPrice,model.shopAdderss,model.identify          ,name,companyName,num,index];
    if (!success) {
        NSLog(@"%@",[_dataBase lastErrorMessage]);
    }else{
        NSLog(@"修改成功");
