...
- (instancetype)init
{
    if(self=[super init]){
        ...
        //如果创建成功 打开
        if ([_dataBase open]) {
            NSString *createSql = @"create table if not exists CustomerAskPrice(ind integer PRIMARY KEY AUTOINCREMENT,companyID varchar(1024),shopName varchar(1024),shopSize varchar(1024),shopMed varchar(1024),shopColor varchar(1024),shopPrice varchar(1024),shopHuoBi varchar(1024),shopTiaoK varchar(1024),shopAdderss varchar(1024),shopDescribe varchar(1024),shopInfo varchar(1024),shopCustom varchar(1024),shopContent varchar(1024),shopPicture varchar(6000),time  varchar(1024),flag varchar(1024),count varchar(1024) ,cleintName varchar(1024),shopSpecific varchar(1024),askTime varchar(1024)  )";
...
- (void)updateDataModel:(CustomerProductModel *)model number:(int)number  {
    ...
    NSString *sql = [NSString stringWithFormat:@"update CustomerAskPrice set companyID = ?, shopName = ?,shopSize = ?, shopMed = ?, shopColor = ? ,shopPrice = ? ,shopHuoBi = ?, shopTiaoK = ?,shopAdderss= ?, shopDescribe = ? , shopInfo = ? ,shopCustom = ?,shopContent = ?,shopPicture = ?,time = ?,flag = ?,count = ?,cleintName = ?,shopSpecific = ? ,askTime = ? where  ind = ?"];
    NSString *str = [NSString stringWithFormat:@"%d",number];
    BOOL success = [_dataBase executeUpdate:sql,model.companyID,model.shopName,model.shopSize,model.shopMed,model.shopColor,model.shopPrice,model.shopHuoBi,model.shopTiaoK,model.shopAdderss,model.shopDescribe,model.shopInfo,model.shopCustom,model.shopContent,model.shopPicture,model.time,model.flag,model.count,model.cleintName,model.shopSpecific,model.askTime,str];
    if (!success) {
        NSLog(@"%@",[_dataBase lastErrorMessage]);
    }else{
        NSLog(@"修改成功");
...
{
    NSString * isSql = @"select *from CustomerAskPrice where ind =?";
...
- (void)deleteNameFromTable:(int)dataId
{
    ...
    NSString * deleteSql = @"delete from CustomerAskPrice where ind = ?";
...
- (NSArray *)getDataWith:(NSString*)dataId{
    ...
    NSString * resultSql = @"select *from CustomerAskPrice where cleintName =?";
