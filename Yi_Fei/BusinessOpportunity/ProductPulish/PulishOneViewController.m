...
@interface PulishOneViewController ()<TwoTableViewCellDelegate,FooterViewDelegate,SSPopupDelegate,OneFirstViewCellDelegate>{
...
-(void)loadData{
    ...
    for (CustomerModel *model in _listArray) {
        [_nameArray addObject:model.customerName];
    }
    
    //获取单例对象
    _productMg = [FMDBOneList defaultManager];
    //可变数组初始化
    _productArray = [NSMutableArray arrayWithArray:[_productMg getData]];
    _productName = [NSMutableArray array];
    for (shopData *dataModel in _productArray) {
...
FooterView *footerView = [[FooterView alloc] initWithFrame:CGRectMake(0, 0, WIDTH, 150)];
    [footerView.footerBtn setTitle:NSLocalizedString(@"confirmPulish", nil) forState:UIControlStateNormal];
...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.row == 0) {
        ...
        firstCell.titleLabel.text = NSLocalizedString(@"chooseBuyer", nil);
        firstCell.delegate = self;
        firstCell.typeView.userInteractionEnabled = YES;
        return firstCell;
    ...
    if (_nameArray.count > 0) {
        ...
        [selection CreateTableview:_nameArray withSender:nil  withTitle:nil setCompletionBlock:^(int tag){
            ...
            if (![_selectBuyer containsObject:_nameArray[tag]]) {
                ...
                [self getData:NSLocalizedString(@"completeBuyerAdd", nil)];
            }
